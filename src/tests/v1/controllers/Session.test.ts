...
describe('Session controller', () => {
	...
	it('should not be able to login user with wrong e-mail', async () => {
		...
		const login: ApiResponse<void> = await request(app)
			.post('/v1/user/login')
			.send({ email: 'test@jest.com', password });
	...
	it('should login user with login user with username', async () => {
		...
		const login: ApiResponse<void> = await request(app)
			.post('/v1/user/login')
			.send({ username, userhash, password });
	...
	it('should not be able to login user with wrong username', async () => {
		...
		const login: ApiResponse<void> = await request(app)
			.post('/v1/user/login')
			.send({ username: 'jest', userhash, password });
	...
	it('should not be able to username login user with wrong password', async () => {
		...
		const login: ApiResponse<void> = await request(app)
			.post('/v1/user/login')
			.send({ username, userhash, password: 'jest' });
	...
	it('should not be able to login with invalid email', async () => {
		...
		expect(login.body).toBe('Invalid email.');
	});
