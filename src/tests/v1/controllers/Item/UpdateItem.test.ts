...
describe("Read item", () => {
  ...
  it("should make a category search", async () => {
    const { status, body }: ApiResponse<Item[]> = await request(app).get("/v1/item").query({ category: "tests" })
  ...
  it("should make property search", async () => {
    const { status, body }: ApiResponse<any[]> = await request(app).get("/v1/item").query({ property: "id", sort: "asc" })
  ...
  it("should make a pagination search", async () => {
    const { status, body }: ApiResponse<Item[]> = await request(app).get("/v1/item").query({ page: 0, quantity: 1 })
  ...
  it("should make a category search with pagination", async () => {
    const { status, body }: ApiResponse<Item[]> = await request(app).get("/v1/item").query({ page: 0, quantity: 15, category: "tests" })
    expect(status).toBe(202)
  })

  it("should make property search with pagination", async () => {
    const { status, body }: ApiResponse<any[]> = await request(app).get("/v1/item").query({ page: 0, quantity: 15, property: "id", sort: "desc" })
